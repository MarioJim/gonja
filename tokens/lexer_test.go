...
var lexerCases = []struct {
	...
	{"basic math expression", "{{ (a - b) + c }}", []tok{
		varBegin, space,
		lParen, name("a"), space, {tokens.Sub, "-"}, space, name("b"), rParen,
		space, {tokens.Add, "+"}, space, name("c"),
		space, varEnd,
		EOF,
	}},
	...
	{"Mixed content", "{# comment #}{% if foo -%} bar {%- elif baz %} bing{%endif    %}", []tok{
		...
		data(" bing"),
		blockBegin, name("endif"), {tokens.Whitespace, "    "}, blockEnd,
		EOF,
	}},
	{"mixed tokens with doubles", "{{ +--+ /+//,|*/**=>>=<=< == }}", []tok{
		varBegin,
		space,
		{tokens.Add, "+"}, {tokens.Sub, "-"}, {tokens.Sub, "-"}, {tokens.Add, "+"},
		space,
		{tokens.Div, "/"}, {tokens.Add, "+"}, {tokens.Floordiv, "//"},
		{tokens.Comma, ","},
		{tokens.Pipe, "|"},
		{tokens.Mul, "*"},
		{tokens.Div, "/"},
	...
	{"Unbalance over end block", "{{ ({a:b, {a:b}}) }}", []tok{
		varBegin, space,
		lParen,
		lBrace, name("a"), {tokens.Colon, ":"}, name("b"), {tokens.Comma, ","},
		space,
		lBrace, name("a"), {tokens.Colon, ":"}, name("b"), rBrace, rBrace,
		rParen,
		space, varEnd,
		EOF,
	}},
	...
	{"if statement", `{% if 5.5 == 5.500000 %}5.5 is 5.500000{% endif %}`, []tok{
		...
		{tokens.Float, "5.5"}, space, {tokens.Eq, "=="}, space, {tokens.Float, "5.500000"},
		space, blockEnd,
...
func TestRegexpClashingDelimiters(t *testing.T) {
	t.Run("Expression delimiters clashing the regexp parsing", func(t *testing.T) {
		...
		expected, _ := asStreamResult([]tok{
			{tokens.VariableBegin, "[["}, space, name("variable"), space, {tokens.VariableEnd, "]]"},
			{tokens.BlockBegin, "[%"}, space, name("block"), space, {tokens.BlockEnd, "%]"},
			{tokens.CommentBegin, "[#"}, data(" comment "), {tokens.CommentEnd, "#]"},
			EOF,
		})
